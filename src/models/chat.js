...
exports.getHomeChat = (id, cb) => {
  db.query('SELECT m.*, users.phone_number, users.display_name, users.image from chats m left join chats m1 on (((m.id_sender = m1.id_sender and m.id_receiver = m1.id_receiver) or (m.id_sender = m1.id_receiver and m.id_receiver = m1.id_sender ) ) and case when m.created_at = m1.created_at then m.id < m1.id else m.created_at < m1.created_at end ) INNER JOIN users on (m.id_sender = users.id OR m.id_receiver = users.id) WHERE users.id != ? AND m1.id is null and ? in(m.id_sender, m.id_receiver) ORDER BY created_at DESC', [id, id], cb)
}
